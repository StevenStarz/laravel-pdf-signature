...
function getOutputScale(ctx) {
  const devicePixelRatio = Math.min(1, window.devicePixelRatio) || 1;
...
// console.log(firstVisibleElementInd, views, loadedViewIds.length, loadPerPage);
//     if (loadedViewIds.length != 0 && (loadedViewIds.length / loadPerPage) == 0) {
//         continue;
//     } else {
//         if (!loadedViewIds.includes(view.id)) {
//             loadedViewIds.push(view.id);
//         }
//     }
//     // console.log(loadedViewIds);
  ...
  forceRendering(currentlyVisiblePages) {
    ...
    if (pageView) {
      this.#ensurePdfPageLoaded(pageView).then(() => {
        this.renderingQueue.renderView(pageView, this);
      });
      return true;
    }
...
class PDFPageView {
  ...
  reset({
    ...
    for (let i = childNodes.length - 1; i >= 0; i--) {
      ...
      // testing bro
      if ($(node).hasClass("loadingIcon") || $(node).hasClass("canvasWrapper")) {
        node.remove();
      }
  ...
  draw(cPdfViewer) {
    ...
    const paintTask = this.renderer === _ui_utils.RendererType.SVG ? this.paintOnSvg(canvasWrapper) : this.paintOnCanvas(canvasWrapper, cPdfViewer);
  ...
  paintOnCanvas(canvasWrapper, cPdfViewer) {
    ...
    var cPdfPage = this.pdfPage;


    if (! $("#" + 'page-' + this.pdfPage.pageNumber + '-canvas').length) {
        canvas.hidden = true;
        ...
        renderTask = this.pdfPage.render(renderContext);
        ...
        renderTask.promise.then(function () {
            if (! $("#" + 'page-' + cPdfPage.pageNumber + '-canvas').length) {

                var index = cPdfPage.pageNumber - 1;
                var background = canvas.toDataURL({
                    enableRetinaScaling: false
                });
                const beforeFabricWidth = Number(canvas.style.width.replace("px", ""));
                const beforeFabricHeight = Number(canvas.style.height.replace("px", ""));

                var secondCanvas = document.createElement("canvas");
                secondCanvas.width = canvas.width;
                secondCanvas.height = canvas.height;
                secondCanvas.style.width = canvas.style.width;
                secondCanvas.style.height = canvas.style.height;
                secondCanvas.classList = "pdf-canvas";
                secondCanvas.id = 'page-' + cPdfPage.pageNumber + '-canvas';


                $(canvasWrapper).parent().prepend(secondCanvas);

                var fabricObj = new fabric.Canvas(secondCanvas.id, {
                    freeDrawingBrush: {
                        width: 1,
                        color: pdfAnnotate.color
                    },
                ...
                pdfAnnotate.fabricObjects.push(fabricObj);
                if (typeof pdfAnnotate.pdfAnnotateOptions.onPageUpdated == 'function') {
                    fabricObj.on('object:added', function() {
                        var oldValue = Object.assign({}, pdfAnnotate.fabricObjectsData[index]);
                        pdfAnnotate.fabricObjectsData[index] = fabricObj.toJSON()
                        pdfAnnotate.pdfAnnotateOptions.onPageUpdated(index + 1, oldValue, pdfAnnotate.fabricObjectsData[index])
                    })
                }

                fabricObj.on("mouse:down", function () {
                    if (fabricObj.getActiveObjects().length > 0) {
                        $("#delete-signature-btn").prop("disabled", false);
                        $("#delete-signature-btn").removeClass("btn-default").addClass("btn-danger");
                    } else {
                        pdfAnnotate.fabricObjects.forEach((obj) => {
                            obj.selection = false;
                            obj.evented = false;
                ...
                $(fabricObj.upperCanvasEl).click(function(event) {
                    pdfAnnotate.active_canvas = index;
                });
                ...
                $(fabricObj.wrapperEl).css({
                    "width": beforeFabricWidth,
                    "height": beforeFabricHeight,
                })
                $(fabricObj.upperCanvasEl).css({
                    "width": beforeFabricWidth,
                    "height": beforeFabricHeight,
                })
                $(fabricObj.lowerCanvasEl).css({
                    "width": beforeFabricWidth,
                    "height": beforeFabricHeight
                })
                ...
                pdfAnnotate.pages_rendered++;

                var pdfPercentageLoaded = ((pdfAnnotate.pages_rendered / pdfAnnotate.number_of_pages) * 100);
                $("#documentPercentageLoaded").text("PDF loaded: " + Math.round(pdfPercentageLoaded) + "%");

                if (((pdfAnnotate.pages_rendered / pdfAnnotate.number_of_pages) * 100) > 1)  {
                    pdfAnnotate.pdfAnnotateOptions.ready();
                }
            }
